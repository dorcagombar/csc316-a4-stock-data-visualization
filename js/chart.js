class Chart {
    constructor(selector, boxSize) {
        ...
        // DEFINE CLIP PATHS
        // Clip path for the main chart area
    ...
    onZoom(event) {
        ...
        // ⭐ FIX: Reduced ticks on zoom to prevent overlap
    ...
    onBrush(event) {
        ...
        // X Axis (Main chart area)
        const xAxis = d3.axisBottom(this.x).ticks(5); // ⭐ FIX: Reduced ticks on brush to prevent overlap
        ...
        this.rsiAxisG.call(d3.axisBottom(this.x).ticks(5)); // ⭐ FIX: Reduced ticks on brush to prevent overlap
    ...
    render(data, opts = {}) {
        ...
        this.totalHeight = this.height + this.rsiHeight + this.rsiGap;
        ...
        const xAxis = d3.axisBottom(this.x).ticks(5); // ⭐ FIX: Reduced ticks on render for small ranges
        const yAxis = d3.axisLeft(this.y).ticks(6);
        const yVolatAxis = d3.axisRight(this.yVolat).ticks(6).tickFormat(d3.format(".1%"));

        this.xAxisG.call(xAxis);
        this.yAxisG.call(yAxis);
        this.yVolatAxisG.call(yVolatAxis);
        ...
        // RSI axis
        this.rsiAxisG.call(d3.axisBottom(this.x).ticks(5)); // ⭐ FIX: Reduced ticks on render for small ranges
