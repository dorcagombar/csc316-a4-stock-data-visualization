class Chart {
    ...
    updateIndicators(isZoomOrBrush = false) {
        ...
        this.volumeGroup.selectAll("rect").data(this.showVolume ? data : [])
            .join(
        ...
        this.smaGroup.selectAll("path").data(this.showSMA ? smaLines : [])
            .join(
                enter => enter.append("path")
                    ...
                    .style("opacity", 0)
                    .call(enter => enter.transition().duration(duration).style("opacity", 1)), // Fade in
                update => update.transition().duration(duration)
                    .attr("d", d => smaGenFactory(d.field)(d.data))
                    .attr("stroke-dasharray", d => d.style) // Reapply style on update
                    .style("opacity", 1),
                exit => exit.transition().duration(duration)
        ...
        this.volatilityGroup.selectAll("path").data(this.showVolatility ? [data] : [])
            .join(
                enter => enter.append("path")
                    .attr("stroke", "purple")
                    .attr("fill", "none")
                    ...
                    .call(enter => enter.transition().duration(duration)
                        .attr("stroke-dashoffset", 0)), // Wipe left-to-right

                update => update
                    .attr("d", volGen) // 1. Update the path shape first
                    .each(function (d) {
                        // 2. Recalculate and set stroke properties for the new path shape
                        const length = this.getTotalLength();
                        d3.select(this)
                            .attr("stroke-dasharray", length)
                            .attr("stroke-dashoffset", length); // Start from a fully wiped state
                    })
                    .call(update => update.transition().duration(duration)
                        .attr("stroke-dashoffset", 0)), // 3. Transition to fully visible (wipe in)

                exit => exit.transition().duration(duration)
                    .attr("stroke-dashoffset", function () {
                        return this.getTotalLength();
                    }) // Wipe right-to-left (off-screen)
                    .remove()
            );
        ...
        if (isRsiVisible) {
            this.rsiGroup.style("display", null).transition().duration(duration).style("opacity", 1);

            // RSI Line
            this.rsiGroup.selectAll("path.rsi-line").data([data])
                .join(
                    enter => enter.append("path").attr("class", "rsi-line")
                        .attr("stroke", "#d62728").attr("fill", "none").attr("stroke-width", 1.5)
                        .attr("d", this.rsiLine)
                        .each(function (d) {
                            const length = this.getTotalLength();
                            d3.select(this)
                        ...
                        .call(enter => enter.transition().duration(duration).attr("stroke-dashoffset", 0)),
                    update => update.attr("d", this.rsiLine)
                        .call(update => {
                            // When zooming/panning, duration is 0. Just set the path.
                            if (duration === 0) {
                                update
                                    .attr("stroke-dasharray", null) // Remove dasharray/offset during zoom/pan
                                    .attr("stroke-dashoffset", null);
                            } else {
                                // For a normal redraw (toggle), re-apply the wipe-in transition
                                update
                                    .each(function (d) {
                                        const length = this.getTotalLength();
                                        d3.select(this)
            ...
            // Guide lines (Fades)
            this.rsiGroup.selectAll(".rsi-guide").data([70, 30])
                .join(
                    enter => enter.append("line").attr("class", "rsi-guide")
                        .attr("x1", 0).attr("x2", this.width)
                        .attr("y1", d => this.rsiY(d)).attr("y2", d => this.rsiY(d))
                        .attr("stroke", "#999").attr("stroke-dasharray", "4,4")
                        .style("opacity", isZoomOrBrush ? 1 : 0)
                        .call(enter => enter.transition().duration(duration).style("opacity", 1)),
                    update => update.transition().duration(duration)
                        .attr("x1", 0).attr("x2", this.width)
                        .attr("y1", d => this.rsiY(d)).attr("y2", d => this.rsiY(d))
                        .style("opacity", 1),
            ...
            this.rsiGroup.style("display", null);
        } else {
            // Wipe out line first, then fade group
            this.rsiGroup.selectAll("path.rsi-line").transition().duration(duration)
                .attr("stroke-dashoffset", function () {
                    return this.getTotalLength();
                });

            this.rsiGroup.selectAll(".rsi-guide").transition().duration(duration).style("opacity", 0);

            this.rsiGroup.transition().duration(duration)
                .style("opacity", 0)
                .on("end", function () {
                    d3.select(this).style("display", "none");
                });
    ...
    render(data, opts = {}) {
        ...
        // **MODIFIED BENCHMARK DRAWING FOR FADE-IN**
        this.benchmarkGroup.selectAll("path")
            .data(this.currentBenchmarks).join(
            enter => enter.append("path")
                .attr("stroke", (d, i) => this.benchmarkColors[i % this.benchmarkColors.length])
                .attr("fill", "none")
                .attr("stroke-width", 1.5)
                .attr("opacity", 0) // Start at 0 opacity
                .attr("d", benchGen)
                .call(enter => enter.transition().duration(this.transitionDuration).attr("opacity", 0.9)), // Fade in to 0.9
            update => update.attr("d", benchGen)
                .transition().duration(this.transitionDuration).attr("opacity", 0.9), // Keep opacity for update
            exit => exit.transition().duration(this.transitionDuration).attr("opacity", 0).remove() // Fade out on removal
        );
